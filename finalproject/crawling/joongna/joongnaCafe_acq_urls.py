...
def get_postUrls (item, date, status) :
    ...
    with open(f"{item[0]}_joongnaCafe_urls_{status}.txt", 'a') as f:
        for page_num in range(1,101):
            #진행 중임을 확인하기 위한 터미널출력
            print(page_num,flush=True)
            try:
                url = (f"https://cafe.naver.com/ArticleSearchList.nhn?search.clubid=10050146&search.media=0&search. \
                searchdate={date}&search.defaultValue=1&search.exact=&search.include=&userDisplay=50&search. \
                exclude=%B8%C5%C0%D4+%B1%B3%C8%AF+%C4%C9%C0%CC%BD%BA+%BB%F0%B4%CF%B4%D9+%B1%B8%B8%C5&search. \
                option=0&search.sortBy=date&search.searchBy=1&search.searchBlockYn=0&search.includeAll=&search. \
                query={item[1]}&search.viewtype=title&search.page={page_num}")
...
if __name__ == "__main__" :
    ...
    year = 2024
    month = 4
    item = ["iphone13", "%BE%C6%C0%CC%C6%F913"] # item[1] = url 중 query 부분

    Crawling_until_today(year, month, item, 'soldout')
    Crawling_until_today(year, month, item, 'selling')
