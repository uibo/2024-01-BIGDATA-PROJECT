...
def get_postUrls(item, status):
    ...
    with open(f"{item[0]}_joongnaSite_urls_{status}.txt", 'a') as f:
        ...
        for i in range(0,8):
            ...
            for page_num in range(1,126):
                try:
                    ...
                    if status == 'soldout':
                        url = f"https://web.joongna.com/search/{item[1]}?page={page_num}&saleYn=SALE_Y& \
                        sort=RECENT_SORT&minPrice={min_price}&maxPrice={max_price}&productFilterType=APP"
                    ...
                    # 판매중 게시글 탐색하는 경우 모든 게시글의 Url 추출 (판매 완료 상품 미포함하여 검색하는 것이기 때문에 모든 게시글이 판매중인 게시글)
                        ...
                        url = f"https://web.joongna.com/search/{item[1]}?page={page_num}& \
                        sort=RECENT_SORT&minPrice={min_price}&maxPrice={max_price}&productFilterType=APP"
                        res_body = requests.get(url, headers=headers).text
                        soup = BeautifulSoup(res_body, 'html.parser')
                        post_section = soup.find('main').find('div', "w-full text").find('ul').find_next('ul').find_next('ul').find_all("li")
