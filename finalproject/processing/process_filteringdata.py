...
def process_filteringdata(filteringdata_file_path, file_name,product_name):
    ...
    df = df.select(['model', 'feature_list', 'battery', 'upload_date', 'price', 'status', 'location', 'imgUrl', 'url'])
    df = df.toPandas() 
    df.to_csv(output_file_path, header=True, index = False, encoding='utf-8-sig') # Pandas로 쓰기
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        ...
        print("example: python3 process_filteringdata.py hdfs://master01:9000/user/bigdata/ iphone14_filtering.csv iphone14")
        sys.exit(1)
